...
def process_api_response(response_json, sku):
    ...
    try:
        ...
        logging.error(f"A KeyError occurred in process_api_response: {e}", exc_info=True)
        return render_template('error.html', error_message=f"An error occurred while processing product data: Missing key {e}"), 500
    except Exception as e:
        logging.error(f"An unexpected error occurred in process_api_response: {e}", exc_info=True)
